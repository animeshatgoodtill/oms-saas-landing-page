...
const AssetTrackingDocPage = () => {
  const [activeSection, setActiveSection] = useState('overview');

  useEffect(() => {
    const handleScroll = () => {
      const sections = assetTrackingGuide.sections;
    ...
    <div className="min-h-screen bg-white">
        ...
        <Container>
          <div className="max-w-4xl">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
              ...
              <span>/</span>
              <span className="text-foreground">Asset Tracking</span>
            </div>

            <h1 className="font-heading text-4xl md:text-5xl font-bold text-foreground mb-4">
              {assetTrackingGuide.title}
            </h1>
            <p className="text-xl text-muted-foreground leading-relaxed mb-4">
              {assetTrackingGuide.description}
            </p>
            <p className="text-sm text-muted-foreground">
              Last updated: {new Date(assetTrackingGuide.lastUpdated).toLocaleDateString('en-GB', {
                year: 'numeric',
                month: 'long',
                day: 'numeric'
              })}
      ...
      <Container>
        <div className="flex gap-12 relative">
          {/* Sidebar TOC */}
          <DocSidebar sections={assetTrackingGuide.sections} activeSection={activeSection} />

          {/* Content Area */}
          <main className="flex-1 py-12 min-w-0 lg:pl-8">
            <div className="max-w-3xl">
              {assetTrackingGuide.sections.map((section) => (
              ...
              <section className="mt-16 p-8 bg-hero-background rounded-xl border border-border">
                ...
                <p className="text-muted-foreground mb-4">
                  Email support with the job number and the worksheet name. We can see the worksheet&apos;s underlying data and any skipped rows immediately.
                </p>
                <a
...
export default AssetTrackingDocPage;
