...
const DefectsToQuotationDocPage = () => {
  const [activeSection, setActiveSection] = useState('overview');

  useEffect(() => {
    const handleScroll = () => {
      const sections = defectsToQuotationGuide.sections;
    ...
    <div className="min-h-screen bg-white">
        ...
        <Container>
          <div className="max-w-4xl">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
              ...
              <span>/</span>
              <span className="text-foreground">Defects to Quotation</span>
            </div>

            <h1 className="font-heading text-4xl md:text-5xl font-bold text-foreground mb-4">
              {defectsToQuotationGuide.title}
            </h1>
            <p className="text-xl text-muted-foreground leading-relaxed mb-4">
              {defectsToQuotationGuide.description}
            </p>
            <p className="text-sm text-muted-foreground">
              Last updated: {new Date(defectsToQuotationGuide.lastUpdated).toLocaleDateString('en-GB', {
                year: 'numeric',
                month: 'long',
                day: 'numeric'
              })}
      ...
      <Container>
        <div className="flex gap-12 relative">
          {/* Sidebar TOC */}
          <DocSidebar sections={defectsToQuotationGuide.sections} activeSection={activeSection} />

          {/* Content Area */}
          <main className="flex-1 py-12 min-w-0 lg:pl-8">
            <div className="max-w-3xl">
              {defectsToQuotationGuide.sections.map((section) => (
...
export default DefectsToQuotationDocPage;
