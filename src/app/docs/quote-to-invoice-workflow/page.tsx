...
const QuoteToInvoiceWorkflowDocPage = () => {
  const [activeSection, setActiveSection] = useState('overview');

  useEffect(() => {
    const handleScroll = () => {
      const sections = quoteToInvoiceWorkflowGuide.sections;
    ...
    <div className="min-h-screen bg-white">
        ...
        <Container>
          <div className="max-w-4xl">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
              ...
              <span>/</span>
              <span className="text-foreground">Quote to Invoice Workflow</span>
            </div>

            <h1 className="font-heading text-4xl md:text-5xl font-bold text-foreground mb-4">
              {quoteToInvoiceWorkflowGuide.title}
            </h1>
            <p className="text-xl text-muted-foreground leading-relaxed mb-4">
              {quoteToInvoiceWorkflowGuide.description}
            </p>
            <p className="text-sm text-muted-foreground">
              Last updated: {new Date(quoteToInvoiceWorkflowGuide.lastUpdated).toLocaleDateString('en-GB', {
                year: 'numeric',
                month: 'long',
                day: 'numeric'
              })}
      ...
      <Container>
        <div className="flex gap-12 relative">
          {/* Sidebar TOC */}
          <DocSidebar sections={quoteToInvoiceWorkflowGuide.sections} activeSection={activeSection} />

          {/* Content Area */}
          <main className="flex-1 py-12 min-w-0 lg:pl-8">
            <div className="max-w-3xl">
              {quoteToInvoiceWorkflowGuide.sections.map((section) => (
              ...
              <section className="mt-16 p-8 bg-hero-background rounded-xl border border-border">
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  <a
                    ...
                    className="inline-block bg-white text-secondary border-2 border-secondary hover:bg-secondary/10 px-6 py-3 rounded-full font-medium transition-all"
                  >
                    Visit Help Centre
                  </a>
                </div>
...
export default QuoteToInvoiceWorkflowDocPage;
