...
export default function SettingsTourDocsPage() {
    const [activeSection, setActiveSection] = useState<string>('');

    useEffect(() => {
        const handleScroll = () => {
            const sections = settingsTourGuide.sections;
        ...
        <Container>
            <div className="py-16 md:py-24">
                <div className="max-w-7xl mx-auto flex gap-8">
                    {/* Sidebar */}
                    <DocSidebar sections={settingsTourGuide.sections} activeSection={activeSection} />

                    {/* Main Content */}
                    <div className="flex-1 max-w-3xl">
                        {/* Header */}
                        <div className="mb-8">
                            <h1 className="text-4xl md:text-5xl font-bold mb-4">{settingsTourGuide.title}</h1>
                            <p className="text-xl text-gray-600 mb-2">{settingsTourGuide.description}</p>
                            <p className="text-sm text-gray-500">Last updated: {settingsTourGuide.lastUpdated}</p>
                        </div>

                        {/* Sections */}
                        <div className="space-y-8">
                            {settingsTourGuide.sections.map((section, index) => (
                        ...
                        <div className="mt-12 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-8">
                            ...
                            <p className="text-gray-700 mb-4">
                                If you need help finding a specific setting or understanding what a configuration option does,
                            ...
                            <div className="grid md:grid-cols-2 gap-4">
                                ...
                                >
                                    ...
                                    <p className="text-sm text-gray-600">
                                        Engineer&apos;s guide to completing jobs in the field
                                    </p>
                                </Link>
                                <Link
                                ...
                                >
                                    ...
                                    <p className="text-sm text-gray-600">
                                        Creating and issuing certificates from completed jobs
                                    </p>
                                </Link>
                                <Link
                                    href="/docs/invoicing"
                                    className="block p-4 border border-gray-200 rounded-lg hover:border-blue-500 hover:bg-blue-50 transition"
                                >
                                    <h4 className="font-bold text-blue-600 mb-2">Invoicing</h4>
                                    <p className="text-sm text-gray-600">
                                        Converting completed jobs into customer invoices
                                    </p>
                                </Link>
                                <Link
                                    ...
                                    className="block p-4 border border-gray-200 rounded-lg hover:border-blue-500 hover:bg-blue-50 transition"
                                >
                                    <h4 className="font-bold text-blue-600 mb-2">Quote to Invoice Workflow</h4>
                                    <p className="text-sm text-gray-600">
                                        End-to-end workflow from quote to job to invoice
                                    </p>
                                </Link>
                            </div>
                        </div>
