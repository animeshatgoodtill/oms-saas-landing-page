...
heroImage: "/images/blog/eicr-codes.png"
---
...
- **Can't mark an EICR as "Satisfactory"** if C1 or C2 observations are present — see how our [digital certificates](/features/certificates) work
- **Observations link to remedial quotes** — record a C2 on the EICR and the office can open a remedial quote pre-filled with it in one click, using our [quotation workflow](/features/quotations). Nothing is quoted automatically, so you check the price before it goes out
- **Remediation tracking** — each observation shows when it has been quoted and can be marked remediated once the work is done
...
Separately from the EICR codes, engineers log job defects on-site in the [field service app](/features/field-service) with Opscel's own severities (Observation, Minor, Major, Critical), and the office sees them on the job as soon as they sync. No waiting for paperwork.
