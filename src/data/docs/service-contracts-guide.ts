...
export const serviceContractsGuide: IDocGuide = {
    ...
    sections: [
        ...
        },
        {
            ...
            subsections: [
                {
                    content: '<p class="mb-4">Where the site has an asset register and your plan includes the asset module, the equipment is <strong>already listed</strong> on that worksheet when the engineer opens it, each row tagged <em>from site register</em>. It is filtered to the equipment types that worksheet manages, and each item appears on one worksheet only even when the visit carries two.</p><p class="mb-4">On a site with no register, what the engineer recorded last time carries forward instead, tagged <em>from last visit</em>.</p><p class="mb-4"><strong>Large site?</strong> Add a <strong>visit plan</strong> to the contract - split by zone, floor, building or evenly - and each visit\'s job pre-loads only its share, the engineer sees "Visit 2 of 4 - 9 devices this visit", anything missed rolls forward, and the last visit of the year picks up whatever is still untested. The timeline labels every occurrence "Visit k of N". How it works: <a href="/docs/asset-lifecycle#visit-plan" class="text-secondary hover:underline">the visit plan model</a>.</p>'
                },
                {
